...
#define CHECK_APPROX_EQ(a, b, tol)                                             \
  do {                                                                         \
    ...
    if (std::fabs(_a - _b) > (tol)) {                                          \
      std::fprintf(stderr, "FAIL %s:%d: |%.6g - %.6g| > %.6g\n", __FILE__,     \
                   __LINE__, _a, _b, static_cast<double>(tol));                \
      ++g_failures;                                                            \
    }                                                                          \
  } while (0)
...
void test_matrixview_const_conversion() {
  ...
  gemm_y::MatrixView<const float, gemm_y::Space::Host> ro =
      mut; // T -> const T: OK
...
void test_copy_roundtrip_full() {
  ...
  CUDA_CHECK(
      cudaMemcpy(h2.data(), d.data(), d.bytes(), cudaMemcpyDeviceToHost));
...
void test_cublas_gemm_bf16() {
  ...
  CUDA_CHECK(
      cudaMemcpy(hC.data(), dC.data(), dC.bytes(), cudaMemcpyDeviceToHost));
...
void test_naive_gemm_bf16() {
  ...
  kernel(
      gemm_y::GemmArgs<gemm_y::dtypes::bf16>{dA.view(), dB.view(), dC.view()},
      nullptr);
  CUDA_CHECK(cudaDeviceSynchronize());
...
void test_cublas_gemm_fp16() {
  ...
  constexpr int N = 64;
  gemm_y::Matrix<T, gemm_y::Space::Host> hA =
      gemm_y::Matrix<T, gemm_y::Space::Host>::alloc(N, N);
  gemm_y::Matrix<T, gemm_y::Space::Host> hB =
      gemm_y::Matrix<T, gemm_y::Space::Host>::alloc(N, N);
  gemm_y::Matrix<T, gemm_y::Space::Host> hC =
      gemm_y::Matrix<T, gemm_y::Space::Host>::alloc(N, N);
  ...
  gemm_y::Matrix<T, gemm_y::Space::Device> dA =
      gemm_y::Matrix<T, gemm_y::Space::Device>::alloc(N, N);
  gemm_y::Matrix<T, gemm_y::Space::Device> dB =
      gemm_y::Matrix<T, gemm_y::Space::Device>::alloc(N, N);
  gemm_y::Matrix<T, gemm_y::Space::Device> dC =
      gemm_y::Matrix<T, gemm_y::Space::Device>::alloc(N, N);
...
void test_cublas_gemm_tfloat() {
  using T = gemm_y::dtypes::tfloat; // tfloat = tf32 path (TC), not pedantic
                                    // fp32 (CUDA cores).
  constexpr int N = 64;
  gemm_y::Matrix<T, gemm_y::Space::Host> hA =
      gemm_y::Matrix<T, gemm_y::Space::Host>::alloc(N, N);
...
void test_profiler_run_sweep_small() {
  ...
  // ref_* on the naive row should equal kernel_* on the cuBLAS row at the same
  // N.
