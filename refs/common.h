...
__device__ inline void tma_3d_gmem2smem(int dst, const void *tmap_ptr, int x,
...
template <int CTA_GROUP = 1>
__device__ inline void tcgen05_mma_f16(int taddr, uint64_t a_desc,
...
inline void init_tmap_2d_simple(CUtensorMap *tmap, const nv_bfloat16 *ptr,
                                uint64_t global_height, uint64_t global_width,
                                uint32_t shared_height, uint32_t shared_width,
                                CUtensorMapSwizzle swizzle) {
  constexpr uint32_t rank = 2;
  uint64_t globalDim[rank] = {global_width, global_height};
  uint64_t globalStrides[rank - 1] = {global_width *
                                      sizeof(nv_bfloat16)}; // in bytes
