...
__global__ __launch_bounds__(TB_SIZE) void matmul_v1_kernel(
  ...
  for (int iter_k = 0; iter_k < num_iters; iter_k++) {
    ...
    if (warp_id == 0 && elect_sync()) {
      ...
      if constexpr (TMAP_3D) {
        ...
        for (int k = 0; k < BLOCK_K / 8; k++) {
          ...
          tma_2d_gmem2smem(A_smem + k * BLOCK_M * 16, &A_tmap, off_k, off_m,
                           mbar_addr);
          tma_2d_gmem2smem(B_smem + k * BLOCK_N * 16, &B_tmap, off_k, off_n,
                           mbar_addr);
        }
      }

      constexpr int cp_size =
          (BLOCK_M + BLOCK_N) * BLOCK_K * sizeof(nv_bfloat16);
    ...
    if (warp_id == 0 && elect_sync()) {
      ...
      tcgen05_mma_f16(taddr, make_desc(A_smem, BLOCK_M),
                      make_desc(B_smem, BLOCK_N), i_desc, iter_k);
...
template <int BLOCK_N, int BLOCK_K, bool TMAP_3D>
void matmul_v1_launch(const nv_bfloat16 *A_ptr, const nv_bfloat16 *B_ptr,
  ...
  if constexpr (TMAP_3D) {
    ...
    init_tmap_2d_simple(&A_tmap, A_ptr, M, K, BLOCK_M, 8,
                        CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_NONE);
    init_tmap_2d_simple(&B_tmap, B_ptr, M, K, BLOCK_N, 8,
                        CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_NONE);
  }
...
void matmul_v1a(const nv_bfloat16 *A_ptr, const nv_bfloat16 *B_ptr,
                nv_bfloat16 *C_ptr, int M, int N, int K) {
  matmul_v1_launch<256, 256, false>(A_ptr, B_ptr, C_ptr, M, N, K);
}

void matmul_v1b(const nv_bfloat16 *A_ptr, const nv_bfloat16 *B_ptr,
                nv_bfloat16 *C_ptr, int M, int N, int K) {
