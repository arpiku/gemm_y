...
SweepResult Profiler<T>::run_sweep(const std::vector<int> &sizes) {
  ...
  std::printf("[Profiler] arch=%s dtype=%s kernels=%zu sizes=%zu  "
              "warmup=%d timed=%d\n",
              kArchName, dtypes::name<T>().data(), kernels_.size(),
              sizes.size(), kWarmup, kTimed);
  ...
  for (int N : sizes) {
    ...
    //    its stats are reused as ref_* for every subsequent kernel row at this
    //    N.
    ...
    for (const auto &k : kernels_) {
      ...
      if (!k.supports(args)) {
        std::printf("[Profiler] SKIP N=%4d %-12s unsupported domain\n", N,
                    k.name.c_str());
        continue;
      }
...
#ifndef NDEBUG
      {
        cref_snapshot.assign(
            static_cast<std::size_t>(N) * static_cast<std::size_t>(N), T{0});
...
#ifndef NDEBUG
      {
        ...
        if (std::memcmp(now.data(), cref_snapshot.data(),
                        now.size() * sizeof(T)) != 0) {
          std::fprintf(stderr, "[OOB] kernel '%s' (N=%d) corrupted C_ref!\n",
                       k.name.c_str(), N);
          std::abort();
        }
      }
      ...
      std::printf("[Profiler] N=%4d  %-12s  kernel=%.1f us  ref=%.1f us  "
                  "rel_err=%.3e  PASS\n",
                  N, k.name.c_str(), s.median_ns / 1e3,
                  ref_stats.median_ns / 1e3, err.max_rel);
    }
  }
