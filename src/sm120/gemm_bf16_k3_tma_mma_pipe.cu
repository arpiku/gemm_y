...
// next K tile is transferred asynchronously.
...
__device__ inline void issue_tma_stage(const CUtensorMap *a_map,
...
__device__ inline void load_mma_a(const __nv_bfloat16 *As, int warp_m,
                                  unsigned (&a)[4]) {
  const int tid =
  ...
  // Each ldmatrix lane supplies a 16-byte-aligned row address. The transpose
  // form converts the column-major shared tile into the row-major MMA operand.
...
__device__ inline void load_mma_b(const __nv_bfloat16 *Bs, int column,
                                  unsigned (&b)[2]) {
  const int tid =
      static_cast<int>(threadIdx.y) * kThreadsX + static_cast<int>(threadIdx.x);
  const int lane = tid & (kWarpSize - 1);
  ...
  // B is already column-major for the row.col MMA instruction.
...
__global__ void
k3_tma_mma_pipe_gemm_kernel(const __grid_constant__ CUtensorMap a_map,
  ...
  auto *const Bs = As + kPipelineStages * kTileM * kTileK;
  __shared__ uint64_t barriers[kPipelineStages];
  ...
  for (int k0 = 0; k0 < K; k0 += kTileK) {
    const int next_k0 = k0 + kTileK;
    const int next_stage = stage ^ 1;
    // Start the next transfer before waiting on the current stage. This lets
    // the TMA engine make progress while the current transaction drains and
    // while this stage is consumed by MMA below.
    if (next_k0 < K && tx == 0 && ty == 0) {
      issue_tma_stage(&a_map, &b_map, As + next_stage * kTileM * kTileK,
                      Bs + next_stage * kTileK * kTileN, block_row, block_col,
                      next_k0, &barriers[next_stage]);
    }

    mbarrier_wait(&barriers[stage], phase[stage]);
    phase[stage] ^= 1;

    unsigned a[4];
    load_mma_a(As + stage * kTileM * kTileK, warp_m, a);
    for (int mma_n = 0; mma_n < kMmaCountN; ++mma_n) {
      unsigned b[2];
      load_mma_b(Bs + stage * kTileK * kTileN,
                 warp_n * kWarpTileN + mma_n * kMmaN, b);
...
void encode_map(CUtensorMap *map, const __nv_bfloat16 *pointer, int rows,
  ...
  if (status != CUDA_SUCCESS) {
    ...
    std::fprintf(stderr, "k3_tma_mma_pipe: cuTensorMapEncodeTiled failed: %s\n",
...
bool k3_tma_mma_pipe::supports(const GemmArgs<__nv_bfloat16> &args) {
...
void k3_tma_mma_pipe::operator()(GemmArgs<__nv_bfloat16> args,
                                 cudaStream_t stream) const {
  if (!supports(args)) {
    std::fprintf(stderr,
                 "k3_tma_mma_pipe: unsupported shape or stride; profiler "
                 "should "
  ...
  constexpr std::size_t shared_bytes =
      static_cast<std::size_t>(kPipelineStages) *
  ...
  auto kernel = k3_tma_mma_pipe_gemm_kernel;
