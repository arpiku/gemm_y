...
__global__ __launch_bounds__(TB_SIZE) void matmul_v3_kernel(
...
#pragma nv_diag_suppress static_var_with_dynamic_init
  ...
  const int tma_mbar_addr =
      static_cast<int>(__cvta_generic_to_shared(tma_mbars));
  const int mma_mbar_addr =
      static_cast<int>(__cvta_generic_to_shared(mma_mbars));
  ...
  auto compute = [&](int iter_k) {
    ...
    if (warp_id == 0 && elect_sync()) {
      ...
      {
        tcgen05_mma_f16(taddr, make_desc(A_smem), make_desc(B_smem), i_desc,
                        iter_k);
...
template <int BLOCK_N, int BLOCK_K, int NUM_STAGES>
void matmul_v3_launch(const nv_bfloat16 *A_ptr, const nv_bfloat16 *B_ptr,
...
void matmul_v3(const nv_bfloat16 *A_ptr, const nv_bfloat16 *B_ptr,
               nv_bfloat16 *C_ptr, int M, int N, int K) {
