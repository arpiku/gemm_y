...
constexpr int NUM_WARPS =
    6; // 1 warp for TMA, 1 warp for MMA, and 4 warps for epilogue
...
template <int BLOCK_N, int BLOCK_K, int CTA_GROUP, int NUM_STAGES,
          bool DO_PROFILE>
__global__ __cluster_dims__(CTA_GROUP, 1, 1)
    __launch_bounds__(TB_SIZE) void matmul_v6_kernel(
        const __grid_constant__ CUtensorMap A_tmap,
        const __grid_constant__ CUtensorMap B_tmap, nv_bfloat16 *C_ptr, int M,
        int N, int K, int64_t *profiler_ptr, int num_entries) {
...
#pragma nv_diag_suppress static_var_with_dynamic_init
  ...
  if (warp_id == 0 && elect_sync()) {
    for (int i = 0; i < NUM_STAGES; i++) {
      mbarrier_init(tma_mbar_addr + i * 8,
                    CTA_GROUP); // both CTAs report TMA to CTA0 only
      mbarrier_init(mma_mbar_addr + i * 8,
                    1); // CTA0 reports MMA to BOTH CTAs (multicast)
    }
    for (int i = 0; i < 2; i++) {
      mbarrier_init(mainloop_mbar_addr + i * 8,
                    1); // CTA0 reports mainloop to BOTH CTAs (multicast)
      mbarrier_init(
          epilogue_mbar_addr + i * 8,
          4 * CTA_GROUP); // 4 epilogue warps x both CTAs report to CTA0 only
    }
    asm volatile(
        "fence.mbarrier_init.release.cluster;"); // visible to async proxy
  } else if (warp_id == 1) {
  ...
  auto load = [&](int tma_stage, int mma_phase, int iter_k, int bid_m,
                  int bid_n) {
    ...
    const int mbar_addr =
        (tma_mbar_addr + tma_stage * 8) & 0xFEFFFFFF; // this is on CTA0
    ...
    tma_3d_gmem2smem<CTA_GROUP>(A_smem, &A_tmap, 0, off_m, off_k / 64,
                                mbar_addr);
    tma_3d_gmem2smem<CTA_GROUP>(B_smem, &B_tmap, 0, off_n, off_k / 64,
                                mbar_addr);
  ...
  auto compute = [&](int tma_stage, int tma_phase, int mainloop_stage,
    ...
    asm volatile("tcgen05.fence::after_thread_sync;"); // (why) do we need this?
                                                       // from DeepGEMM
    if constexpr (DO_PROFILE)
      profiler.stop();
    ...
    // we specify the LOCAL A smem and B smem here. the tensor cores hardware
    // will know to fetch data from BOTH CTAs, assuming we use the same offset
    // across CTAs. manually unroll 1st iteration to disable accumulation
    {
      tcgen05_mma_f16<CTA_GROUP>(tmem, make_desc(A_smem), make_desc(B_smem),
                                 i_desc, enable_input_d);
    ...
    for (int k1 = 1; k1 < BLOCK_K / 64; k1++)
      for (int k2 = 0; k2 < 64 / MMA_K; k2++) {
        ...
        uint64_t b_desc =
            make_desc(B_smem + k1 * (BLOCK_N / CTA_GROUP) * 128 + k2 * 32);
        tcgen05_mma_f16<CTA_GROUP>(tmem, a_desc, b_desc, i_desc, 1);
      }
    ...
    asm volatile(
        ...
        "multicast::cluster.b64 [%0], %1;" ::"r"(mma_mbar_addr + tma_stage * 8),
  ...
  auto epilogue = [&](int mainloop_stage, int bid_m, int bid_n) {
    ...
    // we are using warp2-warp5 for epilogue. hence, we need to remap the
    // warp_id for accessing tmem.
    ...
    for (int n = 0; n < BLOCK_N / 8; n++) {
      ...
      nv_bfloat16 *out_ptr = C_ptr + (bid_m * BLOCK_M + epilogue_tid) * N +
                             (bid_n * BLOCK_N + n * 8);
  ...
  if (warp_id == 0 && elect_sync()) {
    // TMA warp
    int tma_stage = 0;
    int mma_phase = 1; // the initial MMA phase is 0, and it is available. so we
    ...
    int epilogue_phase = 1; // the initial epilogue phase is 0, and it is
                            // available. hence we initialize it with 1.
    ...
    for (int this_bid = bid; this_bid < num_tiles; this_bid += num_bids) {
      ...
      asm volatile("tcgen05.commit.cta_group::%2.mbarrier::arrive::one.shared::"
                   "cluster.multicast::cluster.b64 [%0], %1;" ::"r"(
                       mainloop_mbar_addr + mainloop_stage * 8),
                   "h"(cta_mask), "n"(CTA_GROUP)
                   : "memory");
    ...
    for (int this_bid = bid; this_bid < num_tiles; this_bid += num_bids) {
      ...
      if (elect_sync()) {
        const int mbar_addr =
            (epilogue_mbar_addr + mainloop_stage * 8) & 0xFEFFFFFF;
        asm volatile(
            "mbarrier.arrive.release.cta.shared::cluster.b64 _, [%0];" ::"r"(
                mbar_addr)
            : "memory");
      }
...
template <int BLOCK_N, int BLOCK_K, int CTA_GROUP, int NUM_STAGES,
          bool DO_PROFILE>
void matmul_v6_launch(const nv_bfloat16 *A_ptr, const nv_bfloat16 *B_ptr,
  ...
  auto init_tmap_AB = [&](CUtensorMap *tmap, const nv_bfloat16 *ptr,
    ...
    uint64_t globalStrides[rank - 1] = {(uint64_t)K * sizeof(nv_bfloat16),
                                        128}; // in bytes
  ...
  auto this_kernel =
      matmul_v6_kernel<BLOCK_N, BLOCK_K, CTA_GROUP, NUM_STAGES, DO_PROFILE>;
  if (smem_size > 48'000)
...
void matmul_v6(const nv_bfloat16 *A_ptr, const nv_bfloat16 *B_ptr,
               nv_bfloat16 *C_ptr, int M, int N, int K) {
  matmul_v6_launch<256, 64, 2, 7, false>(A_ptr, B_ptr, C_ptr, M, N, K, nullptr,
                                         0);
}

void profile_matmul_v6(const nv_bfloat16 *A_ptr, const nv_bfloat16 *B_ptr,
                       nv_bfloat16 *C_ptr, int M, int N, int K,
                       int64_t *profiler_ptr, int num_entries) {
  matmul_v6_launch<256, 64, 2, 7, true>(A_ptr, B_ptr, C_ptr, M, N, K,
                                        profiler_ptr, num_entries);
}
